...
export default async function handler(
  ...
  switch (method) {
    case "GET":
      const users = await query("SELECT * FROM contacts2;");
    ...
    case "POST":
      ...
      try {
        const result: any = await query(
          "INSERT INTO contacts2 (name, phone, email, address) VALUES (?, ?, ?, ?);",
    ...
    case "PUT":
      ...
      const sql = `UPDATE contacts2 SET ${fields.join(", ")} WHERE id = ?;`;
      values.push(id);
      try {
    ...
    case "DELETE":
      ...
      try {
        await query("DELETE FROM contacts2 WHERE id = ?;", [deleteId]);
